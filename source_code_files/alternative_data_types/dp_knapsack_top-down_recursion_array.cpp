// dp_knapsack_top-down_recursion_array.cpp
//
// 2023-09-30
//
...
public:
    ...
    knapsack_recursive(vector<int>& w, vector<int>& v, int T, long long timeout_input) : w(w), v(v), T(T), timeout_input(timeout_input) {
        ...
        if (picks_alloc_bytes > net_free_memory_guess) {
            ...
            try {
              int * picks;  // https://cplusplus.com/reference/cstdlib/calloc/
              picks = (int*) calloc (picks_alloc,sizeof(int));
              
              // int picks[picks_alloc];
              ...
              cerr << "\n  int picks[picks_alloc] went wrong!" << endl;
              result = -1;
            }
        }
    }
